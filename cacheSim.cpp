...
way::way(const unsigned entries_num, const unsigned block_size): entries_num(entries_num),
tag_size(EFF_ADDRESS_SIZE - entries_num + (pow(2,block_size)/DATA_SIZE)){// TODO check eff_adrr
    entry = new unsigned[entries_num];
};
way::~way(){ delete[] entry; }

void inline way::swap(unsigned address, unsigned set){
    entry[set] = extract(address,tag_size,ADDRESS_SIZE-tag_size);//tag
}

bool way::check(unsigned address, unsigned set){
    unsigned tag = extract(address,tag_size,ADDRESS_SIZE-tag_size);
    return tag == entry[set];
}
...
level::level(unsigned b_s, unsigned s, unsigned a, unsigned w)
    ...
    for (int i = 0; i < assoc; i++)
        ways_vec.emplace_back(way(entries_num, b_size));
    //LRU table init
    for (int i = 0; i < entries_num; i++) {
        LRU_table.emplace_back(vector<unsigned>());
        for (int j = 0; j < assoc; j++) LRU_table[i].emplace_back(j);
    }
}
...
bool level::accessLevel(unsigned address,bool is_wr) {
    unsigned set = extract(address, (size - b_size - assoc),
                           STRAIGHTEN_BITS + (unsigned) pow(2, b_size - 5));
