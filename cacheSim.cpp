...
tag_size(EFF_ADDRESS_SIZE - entries_num + (pow(2,block_size)/DATA_SIZE)){
    for(unsigned i = 0; i < entries_num; i++)entry.insert(pair<unsigned ,unsigned >(i,0));
}
...
bool way::check(unsigned address, unsigned set){
    ...
    return tag == entry.find(set)->second;
}
